...
`}});
	...
	const point = MakeNamedObject(args, {basename:'Point', source:gds.get('ProductObject', {objects:[Z32, Z32]}), description:'A 32b point'}).to;
	const points = MakeNamedObject(args, {basename:'Points', source:gds.get('HomObject', {objects:[N, point]}), description:'A list of points'}).to;
	const property = MakeNamedObject(args, {basename:'Property', source:gds.get('ProductObject', {objects:[N16, str]}), description:'A property on a gds element'}).to;
	const properties = MakeNamedObject(args, {basename:'Properties', source:gds.get('HomObject', {objects:[N, property]})}).to;
	const boundary = MakeNamedObject(args, {basename:'Boundary', source:gds.get('ProductObject', {objects:[layer, datatype, points, properties]}), description:'A polygon on a chip'}).to;

	const ptFlush = MakeNamedObject(args, {basename:'ptFlush', source:one, description:'A path type with square ends flush with their end point'}).to;
	const ptRound = MakeNamedObject(args, {basename:'ptRound', source:one, description:'A path type with round ends'}).to;
	const ptSquare = MakeNamedObject(args, {basename:'ptSquare', source:one, description:'A path type with squares ends extending a half-width beyond their endpoint'}).to;
	const Z32xZ32 = gds.get('ProductObject', {objects:[Z32, Z32]});
	const ptVariable = MakeNamedObject(args, {basename:'ptVariable', source:Z32xZ32, description:'A path type with squares ends extending a specified lengths beyond their endpoints'}).to;
	const pt = gds.get('ProductObject', {objects:[ptFlush, ptRound, ptSquare, ptVariable], dual:true});
	const endType = MakeNamedObject(args, {basename:'EndType', source:pt, description:'The type of a path explains what to do with their endpoints'}).to;
	...
	const angle = MakeNamedObject(args, {basename:'Angle', source:F, description:'The reference is to be rotated upon placement'}).to;
