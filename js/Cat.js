...
class U
{
	...
	static ConvertData(obj, data)	// hom elements have to be converted from objects to their name
	{
		switch(obj.constructor.name)
		{
			...
			case 'ProductObject':
				if (obj.dual)
					return U.IsNumeric(obj) ? data : [data[0], U.ConvertData(obj.objects[data[0]], data[1])];
				else
					return obj.objects.map(o => U.ConvertData(o, data[0]));
			case 'HomObject':
				return data.name;
			case 'NamedObject':
				return U.ConvertData(obj.base, data);
		}
		return null;
	}
	static InitializeData(diagram, obj, data)	// hom elements have to be converted from objects to their objects
	{
		switch(obj.constructor.name)
		{
			case 'ProductObject':
				if (this.dual)
					return [data[0], U.InitializeData(diagram, obj.objects[data[0]], data[1])];
				else
					return obj.objects.map(o => U.InitializeData(diagram, o, data[i]));
			case 'HomObject':
				return diagram.getElement(data);
			case 'CatObject':
			case 'FiniteObject':
			default:
				return data;
...
//			btns.push(D.GetButton3('search', 'Cat.D.toolbar.showSearch()', 'Search in a diagram'));
		btns.push(D.GetButton3('search', 'Cat.D.toolbar.showSearch()', 'Search in a diagram', D.default.button.small,
	...
	search()
	{
		...
		function showElement(elt)
		{
			...
			if (DiagramObject.IsA(elt))
				...
				return `${to.htmlName()}:${to.domain.htmlName()}&rarr;${to.codomain.htmlName()}`;
			else if (DiagramText.IsA(elt))
				return to.description.length < 15 ? to.description : to.description.substring(0,15) + '...';
		}
		const rows = elts.map(elt => H3.tr(H3.td(showElement(elt), {onclick:function(e) { R.diagram.viewElements(elt); }, class:'left'}), {class:'panelElt'}));
			...
			switch (this.type)
			{
				...
				case 'Morphism':
				{
					...
					if (this.suppress)
					{
						...
						from = diagram.placeMorphism(e, to, args.xyDom, args.xyCod, save);
					break;
				}
				case 'Text':
				{
...
${button}
	...
	static Grid(x)
	{
		const d = D.shiftKey && !D.ctrlKey ? 20 * D.default.layoutGrid : D.default.layoutGrid;
		switch (typeof x)
		{
		case 'number':
...
class ElementSection extends Section
{
	...
	add(elt)
	{
		...
		let id = this.getId(diagram.elementId());		// diagram id
		...
		id = this.getId(elt.elementId());		// element id
	...
	update(diagram, name)
	{
		...
		const id = this.getId(elt.elementId());		// element id
		if (!DiagramText.IsA(elt))
		{
			...
			if (R.default.internals)
			{
				const refElt = document.querySelector(`#${id} refcnt`);
				if (refElt)
					refElt.innerHTML = elt.refcnt;
			}
		}
...
class ObjectPanel extends Panel
{
	constructor()
	{
		...
		this.referenceSection = new ReferenceElementSection('References', this.elt, 'diagram-reference-object', 'Objects referenced from this diagram', 'Object');
		this.initialize();
	}
	expand()
	{
...
class MorphismPanel extends Panel
{
	constructor()
	{
		...
		this.referenceSection = new ReferenceElementSection('References', this.elt, 'diagram-reference-morphism', 'Morphisms referenced from this diagram', 'Morphism');
		this.initialize();
	}
	expand()
	{
...
//		return U.SafeId(`el_${this.name}`);
		return Element.SafeName(this.name);
	}
	usesDiagram(diagram)
	{
...
class FiniteObject extends CatObject	// finite, explicit size or not
{
	...
	getSize()
	{
		return 'size' in this ? this.size : super.getSize();
	}
	static Codename(diagram, args)
	{
		if ('size' in args)
...
class ProductEditAction extends Action
{
	constructor(diagram, dual = false)
	{
		const args =
		{
			...
			name:			dual ? 'coproductEdit' : 'productEdit',
			icon:			dual ?  D.svg.add :
`<line class="arrow0" x1="103" y1="216" x2="216" y2="103"/>
<line class="arrow0" x1="216" y1="216" x2="103" y2="103"/>`,
			dual,
	...
	html(e, diagram, ary)
	{
		...
		if (ProductObject.IsA(elt))
		{
			...
			[
				...elt.objects.map((o, i) =>
				{
					...
					const row = [
						H3.td(style),
						H3.td(style),
						H3.td(!used.has(o) ? D.GetButton3('delete3', `Cat.R.Actions.${this.name}.remove(event, this)`, 'Remove', D.default.button.tiny) : '&nbsp;', style),
						H3.td(o.htmlName()),
					];
			...
			top = H3.div([	H3.h4(`Edit a ${this.dual ? 'Cop' : 'P'}roduct`),
							this.table,
							D.GetButton3('edit3', `Cat.R.Actions.${this.name}.action(event, Cat.R.diagram, '${elt.name}')`, 'Commit editing', D.default.button.tiny)]);
			this.updateTable();
		}
	...
	updateRow(row)
	{
		...
		i > 0 && up.children.length === 0 && up.appendChild(D.GetButton3('up', `Cat.R.Actions.${this.name}.up(event, this)`, 'Move down', D.default.button.tiny));
		i < cnt -1 && down.children.length === 0 && down.appendChild(D.GetButton3('down', `Cat.R.Actions.${this.name}.down(event, this)`, 'Move down', D.default.button.tiny));
	}
	updateTable()
	{
				...
				else if (Morphism.IsA(elt))
				{
					...
					if ((FactorMorphism.IsA(elt) && src === oldProd) || (NamedObject.IsA(src) && src.base === oldProd))
					{
						...
						for (let i=0; i<elt.factors.length; ++i)
						{
							...
							factors.map(nuFctr =>
							{
								if (Arry.isArray(oldFctr) && oldFctr[0] === nuFctr)		// is a sub-factor?
								{
...
<line class="arrow0" x1="110" y1="200" x2="240" y2="280" marker-end="url(#arrowhead)"/>`,
	...
	addFactor(root, ...indices)
	{
		if (this.codomainDiv.innerHTML === '')
			this.codomainDiv.innerHTML = H.span(D.GetButton('edit', `Cat.R.Actions.${this.dual ? 'inject' : 'project'}.action(event, Cat.R.diagram, Cat.R.diagram.selected)`,
				'Create morphism'));
...
`${header}return ${U.Token(m.preCurry)}(${preInput})(${postInput});${tail}`;
	...
	getInputHtml(o, value, prefix = '', factor = [], index = null)
	{
...
//					const out = window[U.Token(f)]([`${prefix} ${o.name} ${factor.toString()}`, value !== null ? [0, value] : [1, 0]]);
...
//					if (o.objects.reduce((r, oi) => r && oi.isTerminal(), true))
					if (U.IsNumeric(o))
	...
	html(e, diagram, ary)
	{
		...
		else	// morphism
		{
			...
			else if (DataMorphism.IsA(to))
			{
				...
				function dataRow(d,i)
				{
					if (d !== null)
					{
						const editDataBtn = D.GetButton('edit', `Cat.R.Actions.run.editData(${i})`, 'Set data');
...
//					to.data.forEach(function(val, i) { dataRow(js.getInputHtml(codomain, val, i, [], i), i); });
					html += H.h5('Data In Morphism');
					html += H.table(rows);
//					canMakeData = to.data.size < sz;
					canMakeData = false;
				}
				else
				{
...
//				html += evalCode;
//				html += H.span(D.GetButton('save', `Cat.R.EmitMorphismEvent(Cat.R.diagram, 'update', Cat.R.diagram.getSelected())`, 'Save'), 'buttonBar');
//				html += H.span(D.GetButton('save', `Cat.R.EmitMorphismEvent(Cat.R.diagram, 'update', Cat.R.diagram.getSelected())`, 'Save'), 'buttonBar');
			}
...
//		d.innerHTML = U.HtmlSafe(U.a2s(result[1]));
		const data = U.ConvertData(result[1]);
...
class IndexCategory extends Category
{
	...
	postProcess(diagram)
	{
		this.forEachMorphism(function(from)
		{
			const to = from.to;
			...
			if (DataMorphism.IsA(to))
				...
				{
					to.data.set(i, U.InitializeData(diagram, to.codomain, d));
				});
		});
	}
	process(diagram, data)
...
//if (this.basename === 'data_8')this.data.clear();
//if (this.basename === 'data_6')this.data.clear();
//if (this.basename === 'data_12')this.data.clear();
	}
	help(helped = new Set)
	{
		if (helped.has(this.name))
	...
	getDataSignature()
	{
		...
		{
			sig = U.SigArray([sig, U.Sig(i.toString()), U.Sig(d ? d.toString() : 'null')]);
		});
...
//		const cod = NamedObject.IsA(codomain) ? codomain.base : codomain;
	...
	assemble(e, base)
	{
		...
		scanning = [...references];
		function backLinkTag(m, morGraph, barGraph, tag)
		{
			propagated.add(m);
			let didAdd = false;
...
//			const didAdd = barGraph.graphs[1].hasTag(tag) && backLinkTag(morGraph, barGraph, tag);
			if (barGraph.graphs[1].hasTag(tag))
			{
				if (backLinkTag(fm, morGraph, barGraph, tag))
					fm.domain.codomains.forEach(function(morph) { isConveyance(morph.to) && scanning.push(morph); });
			}
		}
...
addBall(m);
		...
		function formMorphism(domain, currentDomain, ndx = [])
		{
			...
			if (eltRefs.length > 0)
			{
				...
				const codomain = diagram.prod(homers, true);
				const dataMorph = new DataMorphism(diagram, {domain, codomain, data});
				const id = diagram.id(currentDomain);
				const steps = [];
				steps.push(diagram.assy(dataMorph, id));	// CD -> codomain x CD
				//
				...
				//
				steps.push(diagram.coprod(homers.map(hom => diagram.eval(currentDomain, hom.codomain))));
		...
		while(scanning.length > 0)
		{
			...
			const morphism = formMorphism(domain, domain.to, []);
