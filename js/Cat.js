...
//			window.dispatchEvent(new CustomEvent('LoadDiagram', {detail:	{diagram:R.diagram}, bubbles:true, cancelable:true}));
	...
	{
//if (DiagramMorphism.IsA(item))debugger;
...
console.log('LoadEquivalence', item.name, leftSigs, rightSigs);
	...
	{
		R.workers.equality.postMessage({command:'Reload'});
	}
...
<rect x="0" y="0" width="320" height="320" style="fill:${bgColor}"/>`;
	...
	static View(ary)
	{
		let bbox = new D2.Merge(...ary.map(a => a.getBBox()));
	}
	static Paste(e)
	{
		const diagram = R.diagram;
...
//		const copies = diagram.doPaste(e, mouse, D.pasteBuffer);
...
//			H.button('&#9656;&nbsp;', 'clickable', '', '', `onclick="Cat.D.ttyPanel.logSection.replayCommand(event, ${this.logElt.childElementCount})"`) : '') +
...
//		diagram._log = this._log;		// restore log
		D.Status(e, msg);
	}
...
//				this.addAssertion(R.diagram, a);
	...
	deleteAssertion(name)
	{
		...
		if (a)
		{
			window.dispatchEvent(new CustomEvent('Assertion', {detail:	{diagram:this.diagram, command:'delete', name:a.name}}));
			a.decrRefcnt();
			R.diagram.update();
		}
	}
	...
	constructor()
	{
		...
		R.workers.equality.addEventListener('message', function(msg)
		{
			if (msg.data.command === 'Info')
				that.equalityElt.innerText = JSON.stringify(msg.data);
			else
...
//			id:			{value:	diagram.getId(this),								writable:	false},
		});
...
class Assertion extends Element
{
	constructor(diagram, args)
	{
		...
		if (!('description' in nuArgs))
			this.description = `The assertion that the morphisme ${left.map(m => m.to.properName).join()} equal ${right.map(m => m.to.properName).join()}.`;
		Object.defineProperties(this,
		{
...
//			window.dispatchEvent(new CustomEvent('Assertion', {detail:	{diagram:this.diagram, command:'delete', name:this.name}}));
		}
	}
	json()
	{
	...
	removeEquivalence()
	{
		R.RemoveEquivalences(this.name);
		R.ReloadEquivalences();
	}
	/*
...
<line class="arrow0" x1="110" y1="160" x2="280" y2="160" marker-end="url(#arrowhead)"/>`,
	...
	action(e, diagram, ary)
	{
		...
		diagram.log({command:'homRight', domain:domain, morphism:morphism});
	}
	html(e, diagram, ary)
	{
		const from = ary[0];
...
//				rows += D.HtmlRow(m, `onclick="Cat.R.diagram.placeMorphismByObject(event, 'domain', '${from.name}', '${m.name}', true)"`);
				rows += D.HtmlRow(m, `onclick="Cat.R.$Actions.getElement('homRight').action(event, Cat.R.diagram, ['${from.name}', '${m.name}'])"`);
	...
	{
		Cat.R.diagram.placeMorphismByObject(event, 'domain', domain, morphism, save);
	}
	replay(e, diagram, args)
	{
...
<path class="svgfilNone svgstr1" d="M90,190 A120,50 0 1,0 230,190"/>`,};
	...
	doit(e, diagram, items)
	{
		...
		const names = elements.map(e => e.to.name);
		R.RemoveEquivalences(...names);
		R.ReloadEquivalences();
...
<line class="arrow0" x1="120" y1="160" x2="240" y2="160"/>`,
	...
	{
		if (!Assertion.HasForm(diagram, left, right))
...
class Cell extends DiagramCore
{
	...
	setGlow()
	{
		this.svg && this.svg.setAttributeNS(null, 'class', this.properName === '?' ? 'badCell' : 'cellTxt');
	}
	register()
	{
	...
	emphasis(on)
	{
		/*
			const fn = function(m)
			{
				...
				elts.map(e => D.SetClass(e));
			}
			this.left.map(m => fn(m));
			this.right.map(m => fn(m));
//			toggle(...this.getObjects().map(o => o.svg));
					*/
...
//			if (Assertion.IsA(cell.to))
			if (this.getAssertion(cell.signature))
...
`;
	...
	paste(e)
	{
		...
		const copies = this.doPaste(e, mouse, D.pasteBuffer);
		this.deselectAll();
		copies.map(e => this.addSelected(e));
		D.ShowToolbar(e, mouse);
		this.log({command:'paste', elements:D.pasteBuffer.map(e => e.name), xy:{x:mouse.x, y:mouse.y}});
	}
	...
	*/
	emphasis(c, on)
	{
		...
		*/
		...
		D.mouseover = on ? elt : null;
		if (elt && (DiagramMorphism.IsA(elt) || DiagramObject.IsA(elt)))
			elt.emphasis(on);
		else if (this.domain.cells.has(c))
	...
	*/
	...
	updateViewSaveTimer()
	{
		...
		{
console.log('viewSaveTimer', diagram.name);
