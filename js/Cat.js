...
class R
{
	...
	static Initialize()
	{
		try
		{
			...
			const categoryDiagram = new Diagram(R.$CAT, {basename:'category', name:'category', codomain:'Actions', description:'diagram for a category', user:'system'});
			let xy = new D2(300, 300);
			let diagram = categoryDiagram;
			function placeAction(a)
			{
//				diagram.objects.add(a);
			...
			const productDiagram = new Diagram(R.$CAT, {basename:'product', name:'product', codomain:'Actions', description:'diagram for products', user:'system'});
			xy = new D2(300, 300);
			...
			const coproductDiagram = new Diagram(R.$CAT, {basename:'coproduct', name:'coproduct', codomain:'Actions', description:'diagram for coproducts', user:'system'});
			xy = new D2(300, 300);
			...
			const homDiagram = new Diagram(R.$CAT, {basename:'hom', name:'hom', codomain:'Actions', description:'diagram for hom actions', user:'system'});
			xy = new D2(300, 300);
			diagram = homDiagram;
...
//					D.mouseover = diagram.hasOverlap(new D2(from.svg().getBBox()), from.name);
					D.mouseover = diagram.hasOverlap(xy, from.name);
console.log('D.mouseover', D.mouseover);
...
//		localStorage.setItem(U.StorageName(diagram.name), diagram.stringify());
		localStorage.setItem(diagram.name, diagram.stringify());
...
class CategoryPanel extends Panel
{
	...
	update()
	{
		...
		{
			D.navbar.update();
			this.properNameElt.innerHTML = category.properName;
			this.descriptionElt.innerHTML = category.description;
			this.userElt.innerHTML = category.user;
			const isEditable = !category.readonly && category.user === R.user.name;
			...
			const actions = [];
			category.actions.forEach(function(a) { actions.push(U.DeCamel(a.properName)); });
...
class ObjectPanel extends Panel
{
	...
	update()
	{
		...
		if (R.diagram)
		{
			this.diagramObjectSection.setObjects(R.diagram.elements);
			this.categoryObjectSection.setObjects(R.diagram.codomain.elements);
		}
	}
}
...
if (this.basename === '')debugger;
	...
	signature(sig = '')
	{
		return U.sha256(`${sig}${this.diagram.nameSignature()}/${this.basename}`);
	}
	/*
...
class CatObject extends Element
{
	...
	factorButton(data)
	{
		...
		return H.table(H.tr(H.td(H.button(this.properName + H.sub(data.index.join()), '', D.elementId(), data.title,
			`data-indices="${data.index.toString()}" onclick="R.diagram.addFactor('${data.id}', '${data.fname}', '${data.root}', '${data.action}', ${data.index.toString()});${'x' in data ? data.x : ''}"`))));
	}
...
class FiniteObject extends CatObject	// finite, explicit size or not
{
	...
	{
		return Element.Codename(diagram, `#${args.size.toString()}`);
	}
...
//		const m = this.addFactorMorphism(from.to, U.GetFactorsById('codomainDiv'));
		const m = FactorMorphism.Get(diagram, from.to, U.GetFactorsById('codomainDiv'));
	...
	html(e, diagram, ary)
	{
		...
		const html = H.h4('Create Factor Morphism') +
			...
			// TODO fix D.elementId()
					H.button('1', '', D.elementId(), 'Add terminal object', `onclick="R.diagram.addFactor('codomainDiv', 'selectedFactorMorphism', '#1', '', -1)"`) +
					to.factorButton({action:this.name, fname:'selectedFactorMorphism', root:to.name, index:[], id:'codomainDiv', op:'product'}) +
	...
	html(e, diagram, ary)
	{
		...
		if (isEditable && to.constructor.name === 'CatObject' && to.refcnt === 1)
		{
			html += H.div('Convert generic object to a finite object') +
				D.GetButton('edit', `R.$Actions.getElement('finiteObject').action(event, R.diagram, R.diagram.selected)`, 'Convert to finite object', D.default.button.tiny);
		}
		else
		{
			...
			if (isEditable)
			{
				html += D.GetButton('edit', `R.$Actions.getElement('finiteObject').action(event, R.diagram, R.diagram.selected)`,
...
//		codXY = codXY.add(new D2(this.codomain).subtract(this.domain));
...
class DiagonalMorphism extends Morphism
{
	constructor(diagram, args)
	{
		...
		nuArgs.basename = DiagonalMorphism.Basename(nuArgs.domain, nuArgs.count);
	...
	{
		return Element.Codename(diagram, DiagonalMorphism.Basename(domain, count));
	}
...
//			throw 'no basename for diagram';
		...
		nuArgs.domain = new IndexCategory(diagram, {basename:indexName, description:`index category for diagram ${nuArgs.name}`});
...
//		this.morphisms = new Map();
	...
	*/
	...
	{
		const elts = D.topSVG.querySelectorAll('.object, .morphTxt, .morphism, .diagramText');
		let r = null;
...
//		const m = this.addFactorMorphism(from.to, U.GetFactorsById('codomainDiv'));
	...
	*/
	static Codename(args)
	{
		...
		return `/${args.user}/${codomainName}/${args.basename}`;
	}
}
