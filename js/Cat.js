...
// TODO unused?
	...
	static EmitElementEvent(elt, command)
	{
		if (CatObject.IsA(elt))
			R.EmitObjectEvent(command, elt.name);
		else if (Morphism.IsA(elt))
			R.EmitMorphismEvent(command, elt.name);
		else if (DiagramText.IsA(elt))
			R.EmitTextEvent(command, elt.name);
	}
	static LoadScript(url, fn)
	{
...
class Toolbar
{
	constructor()
	{
		...
		window.addEventListener('mouseenter', function(e) { debugger;D.mouse.savePosition(e); } );
		window.addEventListener('Autohide', function(e)
		{
...
class NewElement
{
	...
	html()
	{
		...
		const action = `Cat.D.newElement.${this.type}.create${this.type}`;
		...
		elts.push(H3.span(D.GetButton3('edit3', `${action}(event)`, this.headline)));
...
<rect x="0" y="0" width="320" height="320" style="fill:${bgColor}"/>`;
	...
	static Mouseover(e, name, on)
	{
		...
		if (from)
		{
			...
			console.log('Mouseover missing element', name);
	}
	static Paste(e)
	{
		if (!D.copyDiagram)
		...
		const pasteElement = function(elt)
		{
			...
			switch(elt.constructor.name)
			{
				...
				case 'DiagramMorphism':
					...
					diagram.addSVG(copy);
					copy.update();
					R.EmitMorphismEvent('new', copy.name);
					break;
				case 'DiagramObject':
			...
			Tab(e)
			{
				...
				if (D.drag)
				{
					if (D.drag && diagram.selected.length === 1)
					{
						...
						if (DiagramObject.IsA(drag) && drag.domains.size + drag.codomains.size === 1)		// drag one arrow
						{
							const draggingCod = drag.domains.size === 0;
							const dragCod = draggingCod ? 'codomain' : 'domain';
							const dragMorph = draggingCod ? [...drag.codomains][0] : [...drag.domains][0];
							const dragTo = drag.to;
							const opObject = draggingCod ? dragMorph.domain.to : dragMorph.codomain.to;
//							opObject = NamedObject.IsA(opObject) ? opObject.base : opObject;
							const target = D.mouseover.to ? D.mouseover.to : drag[dragCod];
console.log('drag prior alternates', drag.dragAlternates);
							if (!drag.dragAlternates)
							{
console.log('drag makeing alternates');
								dragMorph.incrRefcnt();		// goes away on deselect
								const original = dragMorph;
								const {domain, codomain} = draggingCod ? {domain:dragMorph.domain.to, codomain:target} : {domain:target, codomain:dragMorph.codomain.to};
								const homset = diagram.codomain.getHomset(domain, codomain);
//								if (ProductObject.IsA(opObject) && opObject.dual === !draggingCod)	// correct duality for dom or cod
								if (opObject.dual === !draggingCod)	// correct duality for dom or cod
								{
									const factors = opObject.find(target);
									factors.map((f, i) =>
									{
										const fm = diagram.fctr(opObject, f, opObject.dual);	// factor morphism
										if (!homset.includes(fm))
										{
											homset.push(fm);
											!homset.includes(fm) && homset.push(fm);
										}
									});
								}
								const diagMorphisms = [];
								if (Identity.IsA(dragMorph.to) && !homset.includes(dragMorph))
									homset.push(dragMorph.to);		// special fuse to create new morph
								homset.map(altMorph =>
								{
									if (dragMorph.to !== altMorph)
									{
										const nuObj = new DiagramObject(diagram, {to:altMorph[dragCod], xy:drag.getXY()});
										nuObj.orig = U.Clone(drag.orig);	// original drag coordinates
										diagram.addSVG(nuObj);
										nuObj.update();
										nuObj.svg.classList.add('fuseObject', 'glow');
										const args =
										{
											to:altMorph,
											domain: draggingCod ? dragMorph.domain : nuObj,
											codomain: draggingCod ? nuObj : dragMorph.codomain,
										};
										const nuMorph = new DiagramMorphism(diagram, args);
										nuMorph.homSetIndex = -1;
										diagram.addSVG(nuMorph);
										nuMorph.update();
										nuMorph.show(false);
										nuMorph[dragCod].show(false);
										diagMorphisms.push(nuMorph);
										nuMorph.incrRefcnt();	// goes away on deselect
									}
									else
										diagMorphisms.push(dragMorph);
								});
								homset.map(m => m.incrRefcnt());
								drag.dragAlternates = {original, homset, index:homset.indexOf(dragMorph.to), diagMorphisms};
							}
							const homset = drag.dragAlternates.homset;
							if (homset.length > 0)
							{
console.log('drag pre-index', drag.dragAlternates.index);
								if (++drag.dragAlternates.index >= homset.length)
									drag.dragAlternates.index = 0;
console.log('drag index', drag.dragAlternates.index);
								const nxtMorph = drag.dragAlternates.diagMorphisms[drag.dragAlternates.index];
								const nuDragCod = nxtMorph[dragCod];
								if (homset.length > 1)
								{
									dragMorph.show(false);
									dragMorph[dragCod].show(false);
									nuDragCod.dragAlternates = drag.dragAlternates;
									drag.showSelected(false);
								}
								nxtMorph.show();
								nuDragCod.show();
								nuDragCod.showSelected();
								diagram.selected = [nuDragCod];
							}
						}
					}
					else
						diagram.setViewport(D.mouseover.getBBox());
				}
		...
		window.addEventListener('Autohide', function(e)
		{
			...
			if (e.detail.command === 'hide')
				for (const name in D.panels.panels)
					D.panels.panels[name].elt.style.width = '0';
			else
				for (const name in D.panels.panels)
				{
...
class ReferenceDiagramSection extends DiagramSection
{
	...
	add(diagram)
	{
		super.add(diagram, [D.GetButton3('delete3', `Cat.D.RemoveReference(event,'${diagram.name}')`, 'Remove reference diagram')]);
	}
}
...
class ElementSection extends Section
{
	...
	add(elt)
	{
		...
		if (!div)
			div = H3.div({id, class:`panelElt ${DiagramText.IsA(elt) ? '' : 'grabbable'} sidenavRow`, draggable:true, ondragstart, onclick});
		else
			D.RemoveChildren(div);
		if (this.type === 'Object')
		{
			const colspan = R.default.internals ? 2 : 1;
			tds.push(H3.td(H3.tag('proper-name', elt.properName)));
			div.appendChild(	H3.table(	[H3.tr(tds),
											H3.tr(H3.td(H3.tag('description', elt.description), {colspan}))]));
		}
		else if (this.type === 'Morphism')
		{
...
class ObjectPanel extends Panel
{
	constructor()
	{
		...
		this.elt.innerHTML =
			...
			H.h3('Objects');
		this.objectSection = new DiagramElementSection('Objects', this.elt, 'diagram-object', 'Objects in this diagram', 'Object');
		this.referenceObjectSection = new ReferenceElementSection('References', this.elt, 'diagram-reference-object', 'Objects referenced from this diagram', 'Object');
		this.initialize();
	}
	expand()
	{
...
class MorphismPanel extends Panel
{
	constructor()
	{
		...
		this.elt.innerHTML =
			H.table(H.tr(this.expandPanelBtn() + this.closeBtnCell()), 'buttonBarRight') +
			H.h3('Morphisms');
		this.morphismSection = new DiagramElementSection('Morphisms', this.elt, 'diagram-morphism', 'Morphisms in this diagram', 'Morphism');
		this.referenceObjectSection = new ReferenceElementSection('References', this.elt, 'diagram-reference-morphism', 'Morphisms referenced from this diagram', 'Morphism');
		this.initialize();
	}
	expand()
	{
...
class Element
{
	...
	editText(e, attribute, value)
	{
		...
		if (attribute === 'basename')
		{
			...
			this.category.elements.set(this.name, this);
		}
...
//			baseEdit = H.p('Base name: ' + H.tag('basename', this.basename) + baseBtn);
		/*
		return H.div(	H.h4(H.tag('proper-name', this.htmlName()) + pNameBtn) +
								baseEdit +
...
//								H.p(`User: ${this.diagram.user}`), '', id);
		...
		return H.table(
				...
				H.tr(H.td('User', 'left') + H.td(this.diagram ? this.diagram.user : '', 'left')) +
				(R.default.internals ? H.tr(H.td('Internal name:', 'left') + H.td(this.name, 'left')) : '') +
				(R.default.internals ?  H.tr(H.td('Reference count:', 'left') + H.td(H.tag('refcnt', this.refcnt), 'left')) : '') +
				(R.default.internals ? H.tr(H.td('Signature:', 'left') + H.td(this.signature, 'left')) : ''),
				'', id);
	}
	isEditable()
	{
	...
	makeSVG(node, data, first = true)	// data {index, root, dom:name, cod:name, visited, elementId}
	{
		...
		if (this.isLeaf() && this.links.length > 0)
		{
			...
			for (let i=0; i<this.links.length; ++i)
			{
				...
				path.addEventListener('mouseover', function(e){ Cat.D.statusbar.show(event, fs); });
			}
		}
...
class DiagramObject extends CatObject
{
	...
	isIdentityFusible()
	{
		...
		if (this.domains.size === 0 && this.codomains.size === 1)
		{
			...
			m.forEachHier(function(m) { isId = isId && Identity.IsA(m); });
		}
		return isId;
	}
	isFusible(o)
...
//				!InitialObject.IsA(m.codomain)
	...
	html(e, diagram, ary)
	{
		...
		if (elt.constructor.name === 'Morphism' || elt.constructor.name === 'CatObject')
		{
			...
			if (this.isEditable(elt))
				div.appendChild(D.GetButton3('edit3', `Cat.R.Actions.${this.name}.setCode(event, 'element-${this.ext}', '${this.ext}')`, 'Edit code', D.default.button.tiny));
		}
		else
		{
			this.diagram = diagram;
	...
	generateDiagram(diagram)
	{
		...
		this.diagram = diagram;
		let code = '';
		diagram.domain.elements.forEach(function(elt)
		{
			if (Morphism.IsA(elt) || CatObject.IsA(elt))
				code += that.generate(elt.to, generated);
		});
		return code;
	}
...
//	getInput(o, prefix = '', factor = [], namedProperName = '')
	getInput(o, prefix = '', factor = [])
	{
		...
		{
			case 'NamedObject':
				html = this.getInput(o.getBase(), prefix, factor);
				break;
			case 'CatObject':
...
${tail}`;
				...
				case 'TerminalMorphism':
					code += this.getComments(morphism);
					code += morphism.dual ? `${header}${tail}` : `${header}	out = 0;${tail}`;
					break;
			}
		return code;
	}
...
<line class="arrow6" x1="0" y1="160" x2="320" y2="160"/>`,
	...
	action(e, diagram, ary)
	{
		...
		// TODO emit event?
		R.SaveLocal(diagram);
		D.toolbar.show(e);		// TODO add event for this?
	}
	doit(e, diagram, items)
	{
...
if (tokens[1] === 'gdsReader')
{
	switch(tokens[2])
	{
		case 'BndryData':
			args.prototype = 'NamedObject';
			args.source = "hdole/gdsReader/Po{hdole/gds/Layer,hdole/gds/Datatype,hdole/gds/Points,hdole/gds/Properties}oP";
			break;
		case 'PathData':
			args.prototype = 'NamedObject';
			args.source = "hdole/gdsReader/Po{hdole/gds/Width,hdole/gds/EndType}oP";
			break;
		case 'SrefData':
			args.prototype = 'NamedObject';
			args.source = "hdole/gdsReader/Po{hdole/gds/Reflection,hdole/gds/Angle,hdole/gds/Mag}oP";
			break;
		case 'ArefData':
			args.prototype = 'NamedObject';
			args.source = "hdole/gdsReader/Po{hdole/gds/Rows,hdole/gds/Cols,hdole/gds/RowPitch,hdole/gds/ColPitch}oP";
			break;
	}
}
*/
...
class Morphism extends Element
{
	...
	forEachHier(fn)
	{
		if (MultiMorphism.IsA(this))
			this.morphisms.map(m => fn(m));
	}
	static IsA(m)
	{
...
class NamedMorphism extends Morphism	// name of a morphism
{
	...
	loadEquivalences()	// don't call in Morphism constructor since signature may change
	{
		...
		R.LoadEquivalences(this.diagram, this, [this], [this.source]);
	}
	getGraph(data = {position:0})
	{
...
if ('viewport' in this && this.viewport.y === null)this.viewport.y = 0;
	}
	addElement(elt)
	{
	...
	*/
	...
	placeObject(e, to, xyIn, save = true)
	{
		...
		if (save)
		{
			...
			R.SaveLocal(this);
			R.EmitObjectEvent('new', from.name);
		}
		return from;
	}
...
//			xyC = new D2({x:xyD.x + Math.min(D.default.arrow.length, tw), y:xyD.y});
	...
	placeMorphismByObject(e, dir, objectName, morphismName, save = true)
	{
		try
		{
			...
			const xy = D.Grid(D2.Scale(al + Math.abs(cosAngle) * to.textwidth(), {x:cosAngle, y:Math.sin(angle)}).add(fromObj));
	...
	{
//		const homset = this.codomain.getHomset(from.to, target.to);
		...
		if (identFuse)
		{
			...
			const oldTo = m.to;
			const basename = this.getAnon('morph', true);
