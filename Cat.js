...
const Cat =
{
	...
	{
		let btns = '';
		for (const b in display.svg.Buttons)
...
//						H.table(btns.map(row => H.tr(row.map(b => H.td(Cat.display.getButton(b))).join(''))).join(''), 'buttonBar center') +
	...
	barycenter(ary)
	{
		...
		{
			const elt = ary[i];
			if (CatObject.isPrototypeOf(elt) || DiagramElement.isPrototypeOf(elt))
				if (!(elt.name in elts))
					elts[elt.name] = elt;
...
//									dgrm.setObject(targetObject, to);
					...
					//
						...
						Morphism.isPrototypeOf(dragObject.to) &&
						Morphism.isPrototypeOf(targetObject.to) &&
...
Display.prototype.addEventListeners()
{
	...
	{
		if (this.statusbarElt.style.display === 'block' && D2.dist(Cat.statusXY, {x:e.clientX, y:e.clientY}) > 50)
...
function CategoryPanel()
{
	...
	this.elt.innerHTML = H.table(H.tr(display.closeBtnCell('category', false)), 'buttonBarRight') +
...
ObjectPanel.updateRows(dgrm)
{
	let found = {};
	let objects = [];
	for (const [k, obj] of dgrm.codomain.objects)
...
//					(readonly ? '' : Cat.display.getButton('edit', `Cat.getDiagram().editElementText('properNameElt', 'properName')`, 'Edit proper name', Cat.default.button.tiny)));
...
Graph.prototype.svg(dgrm, data, first = true)	// data {index, dom:{x,y, name}, cod:{x,y, name}, visited, elementId}
{
	if (this.isLeaf())
	{
		...
		for (let i=0; i<this.links.length; ++i)
		{
			...
			return `<path data-link="${lnkStr} ${idxStr}" class="string" style="stroke:#${color}AA" id="${linkId}" d="${d}" filter="url(#softGlow)" onmouseover="Cat.status(evt, '${fs}')"/>\n`;
		}
		return svg;
	}
...
CatObject.prototype.getGraph(data = {position:0}, first = true)
{
...
function ProductObject(diagram, args)
{
	...
	this.prototype.seperatorWidth = Cat.textWidth('&times');	// in pixels
	this.setSignature();
}
...
{
	return super.getGraph(this.prototype.name, data, Cat.textWidth('('), Cat.textWidth('&times;'), first);
}
...
Sequence.prototype.Get(diagram, objects)
{
	...
	return obj === null ? new Sequence(diagram, {objects}) : obj;
}
...
function CoproductObject(diagram, args)
{
	...
	this.prototype.seperatorWidth = Cat.textWidth('&times');
	this.setSignature();
}
CoproductObject.getGraph(data = {position:0}, first = true)
{
	return super.getGraph(this.prototype.name, data, Cat.parenWidth, Cat.textWidth('&plus;'), first);
}
...
{
	return super.getGraph(this.prototype.name, data, Cat.bracketWidth, Cat.commaWidth, first)
}
//
	...
	if ('objects' in args)
		Object.keys(args.objects).forEach(function(key)
		{
			if (!this.getObject(key))
			{
				try
				{
					const obj = CatObject.Process(this.diagram, args.objects[key]);
				}
				catch(x)
				{
					errMsg += x;
...
DiagramMorphism.prototype.intersect(bbox, side, m = Cat.default.arrow.margin)
{
	...
	{
	case 'top':
		pnt = D2.segmentIntersect(	x, y, x + w, y, this.domain.x, this.domain.y, this.codomain.x, this.codomain.y);
		break;
	case 'bottom':
		pnt = D2.segmentIntersect(	x, y + h, x + w, y + h, this.domain.x, this.domain.y, this.codomain.x, this.codomain.y);
		break;
	case 'left':
		pnt = D2.segmentIntersect(	x, y, x, y + h, this.domain.x, this.domain.y, this.codomain.x, this.codomain.y);
		break;
	case 'right':
		pnt = D2.segmentIntersect(	x + w, y, x + w, y + h, this.domain.x, this.domain.y, this.codomain.x, this.codomain.y);
		break;
	}
	return pnt;
}
...
MultiMorphism.prototype.getGraph(data = {position:0})
{
	...
	{
		g.componentGraph(dom.graphs[i], i);
		g.componentGraph(cod.graphs[i], i)
	});
}
...
MultiMorphism.prototype.mergeMorphismGraphs(dual = false)
{
	...
	const dom = this.graphs[domNdx];
	const cod = this.graphs[codNdx];
	//
	// for all our comprising graphs...
	//
	graphs.map((g, i) =>
	{
		graph.graphs[domNdx].mergeGraphs({from:g.graphs[domNdx], base:[domNdx], inbound:[], outbound:[codNdx, i]});
		const gCod = g.graphs[codNdx];
		const tCod = this.graphs[codNdx];
		...
		cod.componentGraph(cod.graphs[i], true, i);
	});
	return graph;
}
//
...
{
	return this.mergeMorphismGraphs(graph, true);
}
//
...
{
	return Element.Codenmae(diagram, `Ca{${morphisms.map(m => m.name).join(',')}}aC`);
}
...
FactorMorphism.prototype.getGraph(s, data)
{
	...
	m.factors.map((r, i) =>
	{
		...
		domRoot.unshift(0);
		dom.bindGraph(true, {cod, link:[], tag:'factor', domRoot, codRoot:m.factors.length > 1 ? [1, i] : [1], offset});
	});
...
{
	const nuArgs = Cat.clone(args);
	nuArgs.domain = diagram.codomain.getObject(args.domain);
...
DiagonalMorphism.prototype.Codomain(diagram, domain, limit)
{
	const objects = [];
	let k = this
	return ProductObject.Get(diagram, factors.map(f => domain.getFactor(f)));
...
function FoldMorphism(diagram, args)
{
	...
	Morphism.call(this, diagram, nuArgs);
	this.count = Cat.getArg(args, 'count', 2);
	if (count < 2)
...
FoldMorphism.prototype.Codomain(diagram, domain, limit)
{
	const objects = [];
	let k = this
	return CoproductObject.Get(diagram, factors.map(f => domain.getFactor(f)));
}
FoldMorphism.prototype.Get(diagram, domain, count)
{
	if (count < 2)
		throw 'count is not two or greater';
	const name = FoldMorphism.Codename(domain);
	const m = diagram.codomain.getMorphism(name);
	return m === null ? new FoldMorphism(diagram, {domain, factors}) : m;
}
...
//
StringMorphism.bindGraph(data)	// data: {cod, link, function, domRoot, codRoot, offset}
{
	if (!this.isGraphable())
		return;
	const domRoot = data.domRoot.slice();
...
Diagram.prototype.editSelected(e)
{
	...
	if (DataMorphism.isPrototypeOf(from.to))
	{
		...
		this.updateElementAttribute(from, 'html', to.properName);
...
Diagram.prototype.lambdaBtnForm(domain, codomain, root)
{
	let html = H.h4('Curry A &otimes; B -> [C, D]') +
		...
		(HomObject.isPrototypeOf(codomain) ?
			H.small('Merge to codomain hom') + Cat.display.getButton('codhom', `Cat.getDiagram().toggleCodHom()`, 'Merge codomain hom', Cat.default.button.tiny) + H.br() : '') +
...
Diagram.prototype.createNamedIdentity(e)
{
	try
	{
		...
		const from = this.getSelected();
		if (CatObject.isPrototypeOf(from))
		{
			...
			const obj = this.codomain.getObjectd(Element.Codename(this, basename));
			if (obj)
...
{
	const from = this.getSelected();
	if (Morphism.isPrototypeOf(from))
	{
...
Diagram.prototype.getObjectMorphisms(obj)
{
	const doms = [];
	const cods = [];
	for(const [from, to] of this.morphisms)
	{
		if (from.domain.isEquivalent(obj))
			doms.push(from);
		if (from.codomain.isEquivalent(obj))
			cods.push(from);
	}
...
Diagram.prototype.checkFusible(pnt)
{
	if (this.selected.length === 1)
	{
		...
		if (CatObject.isPrototypeOf(elt))
		{
			if (CatObject.isPrototypeOf(melt))
			{
			...
			elt.showFusible(Morphism.isPrototypeOf(melt) && this.canProductMorphisms(elt, melt));
	}
}
//
...
{
	if (CatObject.isPrototypeOf(dragFrom))
	{
...
{
	return this.selected.filter(s => CatObject.isPrototypeOf(s)).map(o => o.to);
}
Diagram.prototype.getSelectedMorphisms()
{
	return this.selected.filter(s => Morphism.isPrototypeOf(s)).map(m => m.to);
}
...
{
	if (CatObject.isPrototypeOf(from) || Morphism.isPrototypeOf(from))
	{
		if (from.to)
		{
			from.to[attr] = val;
...
Diagram.prototype.activateToolbar(e)
{
	...
	const xy = 'clientX' in e ? {x:e.clientX, y:e.clientY} : Cat.mouse.xy;
	let xyOffset = true;
	...
	else
	{
		...
		if (Morphism.isPrototypeOf(elt))
		{
			const form = this.domain.hasForm(this.selected);
