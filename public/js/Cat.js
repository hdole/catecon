...
class U		// utilities
{
	...
	{
		return elt instanceof IndexObject || elt instanceof IndexMorphism || elt instanceof DiagramComposite || elt instanceof DiagramText;
	}
	...
	addEventListeners()
	{
		...
		window.addEventListener('Morphism', e =>
		{
			...
			if (element instanceof IndexMorphism)
			{
				...
				diagram.domain.loadCells(diagram);
...
class DetachDomainAction extends Action
{
	...
	hasForm(diagram, ary)	// one morphism with connected domain but not a def of something
	{
		if (diagram.isEditable() && ary.length === 1 && ary[0] instanceof IndexMorphism && !(ary[0] instanceof DiagramComposite) && ary[0].refcnt === 1)
		{
			...
			if (soFar)
				return ![...from.domain.nodes].reduce((r, cell) => r || ([...cell.left, ...cell.right].includes(from) && cell.assertion), false);
			return soFar;
		}
		return false;
	}
...
class Cell
{
	constructor(diagram, args)
	{
		...
		D.emitCellEvent(this.diagram, 'new', this);
	}
	json()
	{
		const a =
...
//		svg.onmouseenter = _ => Cat.R.diagram.emphasis(name, true);
	...
	removeSVG()
	{
		...
		{
			const svg = this.diagram.svgBase.querySelector('#' + this.cellId());
			svg && svg.remove();
...
class IndexCategory extends Category
{
	...
	{
		const a = super.json();
		a.cells = [...this.cells.values()].map(cell => cell.json());
		return a;
	}
	clear()
	{
...
class Diagram extends Functor
{
	...
	postProcess()
	{
		...
		this.domain.cells.forEach(cell => cell.update());
	}
	decrRefcnt()
	{
		super.decrRefcnt();
