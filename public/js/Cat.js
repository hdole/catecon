...
class Runtime
{
	...
	setupActions()
	{
		...
		const actionDiagram = new Diagram(this.$CAT, {basename:'Action', codomain:'Actions', description:'actions for a diagram', user:'sys'});
		...
		new ExpressionAction(actionDiagram);
		...
		new ProductEditAction(actionDiagram);
		...
		new ProductEditAction(actionDiagram, true);
...
class Navbar
{
	...
	update()
	{
		...
		if (D.session.mode === 'diagram' && R.diagram && R.diagram.isEditable())
		{
			...
			toolbar.appendChild(D.getIcon(mode, mode, e => D.advanceInputMode(), {title:'Advance mode', id:'navbar-mode-advance', scale}));
			toolbar.style.width = '32px';
		}
	}
}
	...
	loadDiagrams(fn)
	{
		...
		const loader = d => 
		{
			if (diagrams.length > 0)
				Runtime.DownloadDiagram(d, _ => loader(diagrams.shift()));
			else
				fn();
		};
...
class Display
{
	...
	initialize()
	{
		...
		this.loadScript('js/javascript.js', _ => console.log('javascript loaded'));
...
debugger;
	...
	emitIndexElementEvent(diagram, command, element)
	{
		R.default.showEvents && console.log('emit INDEXELEMENT event', {command, name:element.name});
		return window.dispatchEvent(new CustomEvent('IndexElement', {detail:	{diagram, command, element}, bubbles:true, cancelable:true}));
	}
	emitElementEvent(diagram, command, elt)
	{
		if (elt instanceof CatObject)
			...
			this.emitIndexElementEvent(diagram, command, elt);
	}
...
class IndexObject extends CatObject
{
	...
	{
		super.emphasis(on);
		this.svg.querySelectorAll('*').forEach(elt => elt.classList.remove('emphasis2'));
...
class MorphismAssemblyAction extends Action
{
	...
	hasForm(diagram, ary)
	{
		...
		{
			if (diagram.blob && diagram.blob.objects.has(ary[0]) && diagram.blob.morphisms.size > 1)
				return true;
		}
		return false;
	}
...
class ExpressionAction extends Action
{
	...
	hasForm(diagram, ary)
	{
		if (diagram.isEditable() && ary.length === 1 && diagram.blob)
		{
			const item = ary[0];
			if (item instanceof IndexMorphism)
			{
				if (item.to.isBare())
				{
					if (diagram.blob.morphisms.size > 1 && !diagram.blob.hasReference())
						return true;
				}
			}
			else if (item instanceof IndexObject)
...
class DefinitionAction extends Action
{
	...
	html(e, diagram, ary)
	{
		...
		ary.map(elt =>
		{
			if (elt instanceof CatObject && elt.to.isBare())
				objects.add(elt.to)
			else if (elt instanceof Morphism && elt.to.isBare())
				morphisms.add(elt.to);
			else if (elt instanceof Cell)
			{
				[...elt.left, ...elt.right].map(m => m.to.isBare() && morphisms.add(m.to));
				cells.add(elt);
			}
		});
		...
		const doit = elt => D.toolbar.table.appendChild(H3.tr(H3.td({colspan:2}, elt.getHtmlRow())));
	...
	{
		const from = ary[0];
		const elt = this.doit(e, ary);
		diagram.log({command:'definition'});		// TODO
		diagram.antilog({command:'delete'});		// TODO
	}
	doit(e, diagram, ary)
	{
	}
	hasForm(diagram, ary)	// one object
	{
		...
		if (ary.reduce((r, elt) => r && elt instanceof IndexText, true))
			return false;
		const cells = ary.filter(elt => elt instanceof Cell);
		return cells.length > 0 ? cells.reduce((r, cell) => r && r.assertion !== null, true) : false;
	}
}
...
class IndexElement extends Element
{
	...
	getStart(tangent, normal)
	{
		...
		const base = this.start.add(tangent.scale(-width));
		let largeArc = 0;
		...
		const xy = normal.scale(width).add(base).round();
		const xy2 = normal.scale(-width).add(base).round();
		const r = D2.dist(xy, xy2)/2;
		return `M${xy.x},${xy.y} A${r},${r} 0 0 ${largeArc} ${xy2.x},${xy2.y} `;
	}
	getQuantifier()
	{
		return null;
	...
	{
		if (domain instanceof IndexObject && codomain instanceof IndexObject)
...
class DiagramBlob
{
	...
	{
		const ndxObj = elt instanceof IndexObject ? elt : elt.domain;
...
class PullbackAssembly extends MultiMorphism
{
	constructor(diagram, args)
	{
		...
		nuArgs.morphisms = diagram.getElements(args.morphisms);
		nuArgs[source] = diagram.getElement(args[source]);
		nuArgs[target] = diagram.getElement(args[target]);
...
class Diagram extends Functor
{
	...
	canFormBlob(...elts)
	{
		const elements = elts.filter(elt => elt instanceof IndexObject || elt instanceof IndexMorphism);
		if (elements.length === elts.length)
		{
			const blob = new DiagramBlob(elts[0]);
			if (elements.reduce((r, elt) => r && blob.has(elt), true))
				return blob;
		}
		return null;
	}
	deselect(...elts)
	{
		...
		{
			if (!this.blob.has(...this.selected))
				this.blob = null;
		}
		else
			this.blob = this.canFormBlob(...this.selected);
	}
	addSelected(elt)
	{
		if (!this.selected.includes(elt))	// not already selected
		{
			...
			if (this.blob)
			{
				if (elt instanceof IndexText || elt instanceof IndexElement)		// no blob if selected outside of blob
					this.blob = null;
				else if (elt instanceof IndexMorphism && !this.blob.morphisms.has(elt))
					this.blob = null;
				else if (elt instanceof IndexObject && !this.blob.objects.has(elt))
					this.blob = null;
			}
			else if (elt instanceof IndexObject || elt instanceof IndexMorphism)
				this.blob = this.canFormBlob(...this.selected);
			else if (elt instanceof Cell)
				this.blob = this.canFormBlob(elt.left[0].domain);
	...
	emphasis(item, on)
	{
		...
		if (elt)
		{
			...
			else
			{
				...
				if (elt instanceof IndexMorphism || elt instanceof IndexObject)
					elt = elt.to;
				const emphs = [...this.svgRoot.querySelectorAll(`[data-sig="${elt.signature}"] > text, path[data-sig="${elt.signature}"]`)];
				emphs.map(emph => on && elt.svg !== emph ? emph.classList.add('emphasis2') : emph.classList.remove('emphasis2'));
				if (emphs.length > 0)
	...
	assemble(e, base)
	{
		...
		if (morphism)
		{
			if (morphism instanceof Composite)
			{
				...
				R.Actions.zoom(e, this.diagram, compObjs);
			...
			else
			{
				...
				elts.push(from);
				R.Actions.zoom(e, this.diagram, elts);
			}
		}
		else
		{
