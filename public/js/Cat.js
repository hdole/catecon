...
class R
{
	...
	static async SelectDiagram(name, action = null)
	{
		...
		{
			R.EmitCATEvent('default', R.diagram, action);
			return;
		}
		if (isGUI && name)
			D.Busy();
	...
	update()
	{
		...
		const divs = [	H3.div('.navbar-float',
							H3.table('.w100', H3.tr(
								...
								H3.td('.navtxt-link', 'Catecon', {onclick:_ => R.EmitViewEvent('catalog', D.catalog.view)})))),
...
class Toolbar
{
	...
	hide()
	{
		this.element.classList.add('hidden');
		this.closed = true;
		R.diagram && R.diagram.svgRoot.querySelectorAll('.emphasis').forEach(elt => elt.classList.remove('emphasis'));
	}
	reveal()
	{
		this.element.classList.remove('hidden');
	...
	showSelectedElementsToolbar(diagram, btns)
	{
		...
		this.header.appendChild(H3.table(H3.tr(H3.td(btns), H3.td(this.getCloseToolbarBtn(), '.right')), '.w100'));
	}
...
class ElementTool
{
	...
	setupToolbar2()
	{
		...
		const elementBtn = D.getIcon(tType, tType, e => toolbar.showSection(this.type, 'search'), this.type, D.default.button.small, `${this.type}-search-icon`);
		toolbar2.push(elementBtn);
		R.diagram.isEditable() && toolbar2.push(D.getIcon('new', 'edit', _ => toolbar.showSection(this.type, 'new'), 'New', D.default.button.small, `${this.type}-new-icon`));
...
class BpdTool extends ElementTool	// basename, proper name, description
{
	constructor(type, headline)
	{
		...
		Object.defineProperties(this,
		{
			...
			descriptionElt:		{value: null, writable: true},
			toolbar2:			{value: null, writable: true},
		});
		D.ReplayCommands.set(`new${this.type}`, this);
	}
	addNewSection()
...
class MorphismTool extends BpdTool
{
	...
	{
		super.addNewSection();
		const table = D.toolbar.help.querySelector(`div#${this.type}-new`);
	...
	doit(e, diagram, args, select = true)
	{
		try
		{
			...
			else
			{
				...
				to.loadEquivalences();
				from = diagram.placeMorphism(to, args.xyDom, args.xyCod, select, false);
			}
			return from;
		}
		catch(x)
...
class DiagramTool extends BpdTool
{
	...
	{
		const div = H3.div( H3.h4(H3.span(diagram.properName), H3.span('##diagram-properName-edit')),
							H3.span(diagram.basename, '.smallPrint'), H3.span('##diagram-basename-edit'),
							H3.p(H3.span(diagram.description, {title:'Description'}), H3.span({id:'diagram-description-edit'})),
	...
	html(e)
	{
		...
		header.appendChild(this.getHeaderContent(R.diagram));	
		this.setSearchBar();
	}
	setSaveSorter(e)
	{
...
class HomsetTool extends BpdTool
{
	constructor(headline)
	{
		super('Homset', headline);
		Object.defineProperties(this,
		{
	...
	addNewSection()
	{
		super.addNewSection();
		const table = D.toolbar.help.querySelector(`div#${this.type}-new`);
		this.domainElt = H3.span('##new-domain.w100');
		this.codomainElt = H3.span('##new-codomain.w100');
		table.appendChild(H3.tr(H3.td(this.domainElt)));
		table.appendChild(H3.tr(H3.td(this.codomainElt)));
	}
	getMatchingElements()
	{
		const morphisms = R.diagram.getMorphisms().filter(m => m.name.includes(this.filter) &&
			(m.domain.signature === this.domain.signature && m.codomain.signature === this.codomain.signature) &&
			((this.diagramOnly && m.diagram === R.diagram) || !this.diagramOnly));
		const sigs = new Map();
		morphisms.map(m =>
		{
			if (!sigs.has(m.signature))
	...
	doit(e, diagram, args, save = true)
	{
		try
		{
			...
			const to = new Morphism(diagram, args);
			to.loadEquivalences();
			const from = diagram.placeMorphism(to, args.xyDom, args.xyCod, save, false);
...
class D
{
	...
	static Drop(e)	// from panel dragging
	{
		...
		try
		{
			...
			else
			{
				...
				{
					from = diagram.placeMorphism(elt, xy, null, true, false);
	...
	static PlaceComposite(e, diagram, comp)
	{
		...
		diagram.addSelected(diagram.placeMorphism(diagram.comp(...morphisms), indexObjects[0], indexObjects[indexObjects.length -1], false));
		return indexObjects;
	}
...
class DiagramText extends Element
{
	...
	textEditor()
	{
		...
		const onkeydown = e =>
		{
			const nubox = foreign.getBoundingClientRect();
			foreign.setAttribute('width', scale * nubox.width + 'px');
			foreign.setAttribute('height', scale * nubox.height + 'px');
			if (e.key === 'Escape')
				D.closeActiveTextEdit();
			e.stopPropagation();
		};
...
class DiagramObject extends CatObject
{
	...
	getSVG(node)
	{
		...
		svg.setAttributeNS(null, 'data-to', this.to.name);
...
class CopyAction extends Action
{
	...
	action(e, diagram, ary)
	{
		...
		{
			const xy = new D2(from.domain.getXY());
	...
	doit(e, diagram, args)
	{
		...
		if (source instanceof Morphism)
			return diagram.placeMorphism(source, args.xy);
		else if (source instanceof CatObject)
			return diagram.placeObject(source, args.xy, true);
		else if (typeof source === 'string')
			return diagram.placeText(source, args.xy, args.height, args.weight);
	}
	replay(e, diagram, args)
	{
	...
	{
		return diagram.isEditable() && ary.length === 1 && !(ary[0] instanceof Cell)
	}
}
...
class ProductAction extends Action
{
	...
	doit(e, diagram, elements)
	{
		...
		else if (elt instanceof DiagramObject)
		{
			...
			return diagram.placeObject(to, elt);
		}
	}
	replay(e, diagram, args)
	{
...
class MorphismAssemblyAction extends Action
{
	...
	html(e, diagram, ary)
	{
		...
		this.assembler.assemble(e, ary[0]);
...
class HomsetAction extends Action
{
	constructor(diagram, dual = false)
	{
		...
		if (!isGUI)
			return;
		D.ReplayCommands.set(this.basename, this);
		this.newMorphism = new HomsetTool('');
	}
	...
	html(e, diagram, ary)
	{
		...
		{
			const row = m.getHtmlRow();
			row.onclick = e => this.action(e, Cat.R.diagram, m.name, ary[0].name, ary[1].name);
			return row;
		});
		D.toolbar.help.appendChild(H3.h4('Place Morphism'));
		D.toolbar.help.appendChild(H3.table(rows, '##help-homset-morphism-table'));
	}
...
class LambdaMorphismAction extends Action
{
	...
	placeMorphism(adjacent, morphism)
	{
		...
		return diagram.placeMorphism(morphism, xyDom.add(delta), xyCod.add(delta), true, false);
	}
...
class HelpAction extends Action
{
	...
	hasForm(diagram, ary)	// one element
	{
		return ary.length === 1 && !(ary[0] instanceof Cell);
	}
	html(e, diagram, ary)
	{
		const help = D.toolbar.help;
...
class CellAction extends Action
{
	constructor(diagram)
	{
		const args =
		{
			description:	'cell commutativity',
			basename:		'cell',
		};
		super(diagram, args);
		if (!isGUI)
	...
	html(e, diagram, ary)
	{
		...
		D.RemoveChildren(help);
		const cell = ary[0];
		const elts = [H3.h3('Cell')];
		if (cell.assertion)
			elts.push(D.getIcon('delete', 'delete', {onclick:e => Cat.R.Actions.delete(e, diagram, ary)}, 'Delete assertion'));
		else if (cell.commutes === 'unknown')
			elts.push(	H3.p('Assert that the two legs are:'),
						D.getIcon('equal', 'equal', {onclick:e => this.action(e, diagram, ary, true)}, 'Set to equal'),
						D.getIcon('notEqual', 'notEqual', {onclick:e => this.action(e, diagram, ary, false)}, 'Set to not equal'));
		help.appendChild(H3.div(elts));
		const rows = [	H3.tr(H3.td(`Commutativity: ${U.Cap(cell.commutes)}`)),
						H3.tr(H3.th('Left leg:'))];
		cell.left.map(m => rows.push(m.to.getHtmlRow()));
		rows.push(H3.tr(H3.th('Right leg:')));
		cell.right.map(m => rows.push(m.to.getHtmlRow()));
		help.appendChild(H3.table(rows));
	}
	action(e, diagram, ary, equal)
	{
		const a = this.doit(e, diagram, ary, equal);
//TODO		diagram.log({command:this.name, left:left.map(m => m.name), right:right.map(m => m.name)});
		diagram.antilog({command:'delete', elements:[a.name]});
		diagram.userSelectElement(e, a);
	}
	doit(e, diagram, ary, equal, save = true)
	{
		if (!this.hasForm(diagram, ary))
			throw 'cannot form assertion';
		const cell = ary[0];
		const assert = diagram.addAssertion(cell.left, cell.right, equal);
	...
	replay(e, diagram, args)
	{
		...
		const equal = todo; // TODO
		this.doit(e, diagram, left, right, equal, false);
	}
	hasForm(diagram, ary)
	{
		return ary.length === 1 && ary[0] instanceof Cell;
...
class Cell extends DiagramCore
{
	...
	setGlow()
	{
		if (this.svg)	// TODO?
		{
			...
			this.svg.classList.add(this.commutes === 'unknown' ? 'badCell' : 'cellTxt');
		}
	}
	register()
	{
	...
	getSVG(node)
	{
		...
		this.svg = svg;
		this.setGlow();
		this.diagram.selected.filter(elt => elt instanceof Assertion && elt.signature === this.signature).map(assert =>
		{
			assert.svg = this.svg;
			assert.showSelected();
		});
...
//			viewport:					{value:{x:0, y:0, scale:1.0, width:D.Width(), height:D.Height(), visible:false},	writable:true},
		...
		this.postProcess();
		diagram && this.constructor.name === 'Diagram' && R.EmitCATEvent('new', diagram, this);
	}
	decrRefcnt()
	{
		super.decrRefcnt();
	...
	placeText(description, xy, height = '24', weight = 'normal')
	{
	...
	placeElement(elt, position)
	{
		if (elt instanceof CatObject)
			...
			this.placeMorphism(elt, position, null, true, false);
	...
	autoplace()
	{
		...
		unindexed.map(elt =>
		{
			...
			else if (elt instanceof Morphism)
			{
				...
				this.placeMorphism(elt, {x, y}, cod, false, false);
				if (elt.description !== '')
...
class Assembler
{
	constructor(diagram)
	{
		...
		this.sources = new Set();			// objects that are the domains of the composites
		this.references = new Set();		// factor, identity, or terminal morphisms used as references
		this.propagated = new Set();
		this.inputs = new Set();
		this.outputs = new Set();
		this.composites = new Map();	// sources+inputs to array of morphisms to compose
		this.ref2comp = new Map();
	...
	reset()
	{
		...
		this.issues = [];
		this.clearGraphics();
		this.objectListener && window.removeEventListener('Object', this.objectListener);
		// look for editing actions to clear the graphics
		this.objectListener = e =>
		{
			const args = e.detail;
			const diagram = args.diagram;
			if (!R.sync || !diagram || diagram !== R.diagram || !(args.element instanceof DiagramObject) || !this.finished)
				return;
			switch(args.command)
			{
				case 'fuse':
				...
				case 'new':
				case 'move':
					const asmblr = Cat.R.Actions.morphismAssembly.assembler;
					asmblr && asmblr.reset();
					break;
			}
		};
		this.morphismListener && window.removeEventListener('Morphism', this.morphismListener);
		window.addEventListener('Object', this.objectListener);
		this.morphismListener = e =>
		{
			const args = e.detail;
			const diagram = args.diagram;
			if (!R.sync || !diagram || diagram !== R.diagram || !(args.element instanceof DiagramMorphism) || !this.finished)
				return;
			switch(args.command)
			{
				case 'detach':
		...
		window.addEventListener('Morphism', this.morphismListener);
	}
	clearGraphics()
	{
		[...this.diagram.svgRoot.querySelectorAll('.assyError, .assyInput, .assyOutput, .assyReference, .assySource')].map(elt => elt.remove());
	}
	...
	{
		if (Assembler.isProjection(m) && !(m.to instanceof Identity))
			return FactorMorphism.isReference(m.to.factors);
		return false;
	}
	static addBall(title, cls, o)
	{
		if (R.default.debug)
		{
			...
			if (o instanceof DiagramObject)
			{
				...
				o.svg.parentNode.insertBefore(H3.ellipse('.ball', {class:cls, onmouseenter, rx:bbox.width/2 + D.default.margin, cx:o.x, cy:o.y}), o.svg);
			}
			else if (o instanceof DiagramMorphism)
			{
				const bbox = o.svg_name.getBBox();
				const xy = o.getNameOffset();
				o.svg_name.parentNode.insertBefore(H3.ellipse('.ball', {class:cls, onmouseenter, rx:bbox.width/2 + D.default.margin, cx:xy.x, cy:xy.y}), o.svg_name);
			}
		}
	}
	addError(msg, elt)
	{
		Assembler.addBall(msg, 'assyError', elt);
	}
	findBlob(obj)
	{
		this.reset();
		...
		while(scanning.length > 0)	// find loops or homsets > 1
		{
			...
			[...domain.codomains].filter(m => !Assembler.isConveyance(m.to)).length === 0 && this.sources.add(domain);
			const scan = scanning;		// jshint
			domain.domains.forEach(m =>
			{
				this.morphisms.add(m);
				// inputs do not have references
				Assembler.isReference(m) && this.references.add(m);		// candidate reference
				m.makeGraph();
		...
		this.morphisms.forEach(m => !Assembler.isConveyance(m.to) && this.sources.delete(m.codomain)); // remove sources that are outputs
		this.morphisms.forEach(m => Morphism.isIdentity(m.to) && this.sources.has(m.codomain) && this.references.add(m)); // add references that are identities to sources
	}
	...
	//
	static addTag(dm, graph, tag, ndx, issues)
	{
		...
		if (fctr.hasTag(tag) && ndx[0] !== 0)
		{
			if (!fctr.hasTag('Cofactor'))	// cofactor morphs can fold info
			{
				...
				this.issues.push({message:`object has too much ${tag}`, element:dm.codomain});
				return;
			}
		}
		else
	...
	propTag(dm, m, mGraph, barGraph, tag, issues, fn)
	{
		this.propagated.add(dm);
		if (m instanceof MultiMorphism)	// break it down
			m.morphisms.map((subm, i) => this.propTag(dm, subm, mGraph.graphs[i], barGraph, tag, issues, fn));
		else if (Assembler.isConveyance(m))		// copy tag on input links
			mGraph.graphs[0].scan((g, ndx) => g.links.map(lnk => fn(dm, barGraph, tag, lnk)), [0]);
		else		// tag all pins
			mGraph.graphs[1].scan((g, ndx) => fn(dm, barGraph, tag, ndx), [1]);
	}
	mergeObjectTags()
	{
		...
		const tagInfo = dm => this.propTag(dm, dm.to, dm.graph, Assembler.getBarGraph(dm), 'info', this.issues, Assembler.addTag);
		const scanning = [...this.sources];
		const scanned = new Set();
	...
	{
		let isIt = true;
		this.objects.forEach(o => o.assyGraph.noTag('info') && this.inputs.add(o));		// candidate inputs
		let scanning = [...this.references];
		while(scanning.length > 0)
		{
			const fm = scanning.pop();
			const morGraph = fm.graph;
			const barGraph = Assembler.getBarGraph(fm);
			if (barGraph.graphs[1].hasTag('info'))
			{
				const scan = scanning;		// jshint
				const pushConveyance = morph => Assembler.isConveyance(morph.to) && scan.push(morph);
				if (this.backLinkTag(fm, morGraph, barGraph, 'info'))
		...
		//
		scanning = [...this.inputs];
		while(scanning.length > 0)
		{
			const input = scanning.pop();
			const graph = input.assyGraph;
			if (graph.hasTag('info'))
				this.inputs.delete(input);
			const scan = scanning;		// jshint
			const inputReducer = m =>
			{
				if (m.to instanceof FactorMorphism)
				{
					this.inputs.delete(m.codomain);
					scan.push(m.codomain);
				}
			};
			input.domains.forEach(inputReducer);
			const refs = [...input.domains].filter(m => this.references.has(m));
			if (refs.length > 0)
			{
				...
				if (refIds.length === 0)
				{
					...
					{
						// do the factors cover the source?
						const isuCnt = this.issues.length;
						if (this.diagram.isCovered(input.to, factors, this.issues))
							this.inputs.delete(input);
						else if (this.issues.length > isuCnt)
							this.addError('Factor not covered', input);
					}
					else
					{
						this.issues.push({message:'Factor morphism is not a reference', element:r});
						this.addError('Duplicate factors found', input);
					}
				}
			}
		}
		...
		// inputs do not have references
		//
		this.references.forEach(ref => this.inputs.has(ref.domain) && this.references.delete(ref));
	}
	static setTag(dm, graph, tag, ndx)
	{
	...
	infoProp()
	{
		...
		while(scanning.length > 0)
		{
			...
			const domScanner = dm =>
			{
				...
				{
					this.propTag(dm, dm.to, dm.graph, Assembler.getBarGraph(dm), 'info', this.issues, Assembler.setTag);
		...
		this.inputs.forEach(i => i.domains.forEach(m =>
			{
				Assembler.isConveyance(m) && this.references.delete(m);
				this.propTag(m, m.to, m.graph, Assembler.getBarGraph(m), 'info', this.issues, Assembler.setTag);
			}));
	}
	outputScan()
	{
		let isIt = true;
		const scannerNo = (g, ndx) => isIt = isIt && !g.hasTag('info');
		...
		this.objects.forEach(obj =>
		{
			...
			{
				if (Assembler.isProjection(m.to) && this.references.has(m))
					m.codomain.assyGraph.scan(scannerYes);
				else
					isIt = false;
			});
			isIt && obj.codomains.forEach(m =>
			{
				if (Assembler.isConveyance(m.to))
					isIt = m.to.dual ? isIt && !this.sources.has(m.domain) : isIt && !this.references.has(m);
			});
			if (isIt)
				this.outputs.add(obj);
		});
		// drop inputs that are outputs
		this.outputs.forEach(o => this.inputs.has(o) && this.inputs.delete(o));
	}
	showGraphics()
	{
		this.inputs.forEach(i => Assembler.addBall('Input', 'assyInput', i));
		this.references.forEach(r => Assembler.addBall('Reference', 'assyReference', r));
		this.sources.forEach(s => Assembler.addBall('Source', 'assySource', s));
		this.outputs.forEach(o => Assembler.addBall('Output', 'assyOutput', o));
	}
	sourceScan()
	{
					...
					else if (refIds.length === 0)
					{
						...
						if (FactorMorphism.isReference(factors))
						{
							...
							if (!this.diagram.isCovered(src.to, factors, this.issues))
								this.addError('Source not covered', src);
						}
						else
						{
							this.issues.push({message:'Factor morphism is not a reference', element:r});
	...
	cleanSources()		// no input or output is a source
	{
		this.inputs.forEach(i => this.sources.delete(i));
		this.outputs.forEach(i => this.sources.delete(i));
	}
	followComposite(homers, cmp, m)
	{
		...
		if (morphs.length > 0)
		{
			const next = morphs[0];
			cmp.push(next);
			return this.followComposite(homers, cmp, next);
		}
		else
			return cod;
	}
	startComposites(scanning, homers)
	{
		...
		obj.domains.forEach(m =>
		{
			if (!this.references.has(m))
			{
				const cmp = [m];
				comps.push(cmp);
				const cod = this.followComposite(homers, cmp, m);
				scanning.push(cod);
			}
		});
		comps.length === 0 && this.composites.delete(obj);
	}
	setupComposites()
	{
		...
		const scanning = [...this.inputs, ...this.sources];
		while(scanning.length > 0)
			this.startComposites(scanning, homers);
	}
	getDomainAssemblyFactors(domain, ndx)
	{
		const indices = [...domain.domains].filter(m => !m.to.codomain.isTerminal() && this.references.has(m) && Assembler.isProjection(m) && this.ref2ndx.has(m)).map(ref => this.ref2ndx.get(ref));
		for(let i=0; i<indices.length; ++i)
		{
	...
	//			!(preFactor instanceof Identity) && morphs.unshift(preFactor);
				...
				//
				if ([...codomain.codomains].reduce((r, m) => r || (this.references.has(m) && Assembler.isInjection(m) && m.domain.to.isTerminal()), false))
				{
					//
					// replace last morphism in composite with an assembly
					//
					const lastMorph = morphs[last];
					morphs[last] = this.diagram.assy(lastMorph, this.diagram.id(lastMorph.domain));
				}
				//
				...
				//
				let composite = this.diagram.comp(...morphs);
				//
				// map the references to the composite that creates the referenced value
				//
	//			let hasRef = false;
				codomain.codomains.forEach(m =>
				{
					if (this.references.has(m))
					{
						this.ref2comp.set(m, composite);
	//					hasRef = true;
	//					const refs = this.getReferences(m.domain);
					}
				});
				// if the codomain has a reference then it best be a source in order to return the composite
	//			return hasRef && !this.sources.has(codomain) ? null : composite;
	//			return hasRef ? composite : null;
				return this.composites.has(codomain) ? composite : null;
			}).filter(m => m);
		}
		else
			return [];
	}
	assembleElements(morphisms, domain)
	{
		const eltRefs = [...domain.codomains].filter(m => this.references.has(m) && Assembler.isInjection(m) && m.domain.to.isTerminal());
		if (eltRefs.length > 0)
		{
			const homMorphs = [];
			...
			const eltMorphisms = eltRefs.map((eltRef, i) =>
			{
				...
				//
				const homMorphs = [...eltRef.domain.codomains].map(m => this.formMorphism(scanning, m.domain, currentDomain, index));
				const homMorph = this.diagram.assy(...homMorphs);
				const homset = this.diagram.hom(currentDomain, homMorph.codomain);
				//
				...
				if (r instanceof FactorMorphism)
				{
					const f = r.factors[0];
					if (f.length > 1)
						this.issues.push({message:'Factor is too deep', morphism:r});
					elt = r.factors[0][0];
				}
			...
			const codomain = this.diagram.coprod(...homers);
			const dataMorph = new Morphism(this.diagram, {basename:this.diagram.getAnon('select'), domain:domain.to, codomain, data});
			const id = this.diagram.id(currentDomain);
			const steps = [];
			steps.push(this.diagram.prod(dataMorph, id));
			//
			// needs distribution of the state over the various homsets?
			//
			homers.length > 1 && steps.push(this.diagram.dist(this.diagram.prod(codomain, currentDomain), 'right'));
			//
			// get the evaluation maps
			//
			steps.push(this.diagram.coprod(...homers.map(hom => this.diagram.eval(currentDomain, hom.objects[1]))));
			const doit = this.diagram.comp(...steps);
			morphisms.push(doit);
		}
	}
	// recursive
	formMorphism(scanning, domain, currentDomain, index = [])
	{
		...
		//
		[...domain.codomains].map(m => this.references.has(m) && Assembler.isProjection(m.to) && this.ref2ndx.set(m, index));
		//
		...
		if (this.sources.has(domain) || this.outputs.has(domain))
		{
			...
			if (refs.reduce((r, m) => r && this.references.has(m) && this.ref2comp.has(m), true))
			{
		...
		if (morphisms.length > 0)
		{
			...
			//
			this.assembleElements(morphisms, domain);
			assy = this.diagram.assy(...morphisms);
		}
		if (preamble && assy)
	...
	assemble(e, base)
	{
		...
		this.outputScan();
		//
		// remove sources that are inputs or outputs
		//
		this.cleanSources();		// TODO
		this.showGraphics();
		//
		// scan sources for assembly checks
		//
		if (!this.sourceScan())
			return;		// errors found
		//
