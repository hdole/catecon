...
class U		// utilities
{
	...
	{
		return elt instanceof IndexObject || elt instanceof IndexMorphism || elt instanceof DiagramComposite || elt instanceof Assertion || elt instanceof DiagramText;
	}
...
class Runtime
{
		...
		worker.onmessage = msg =>		// process return messages from the worker
		{
			...
			switch(args.command)
			{
				case 'CheckEquivalence':
					...
					cell.removeSVG();
					const assertion = diagram.getAssertion(cell.name);
					if (assertion)
	...
	setupActions()
	{
		...
		const setup = diagram => isGUI && diagram.elements.forEach(action => {this.Actions[action.basename] = action;});
...
args.codomain = 'zf/Set';
	...
	canLoad(name)
	{
		return [...this.getReferences(name)].reverse().reduce((r, d) => r && (this.hasLocalDiagram(d) || this.$CAT.getElement(d)) !== undefined, true);
	}
	loadDiagram(name)	// assumes all reference diagrams are loaded or local and so is immediate
	{
		const setup = ref =>
		{
			let diagram = this.$CAT.getElement(ref);
			if (!diagram)
				diagram = this.readLocal(ref);
		}
		[...this.getReferences(name)].reverse().map(ref => setup(ref));
	...
	loadSigs(diagram, item, leftSigs, rightSigs, equal = true)
	{
		this.workers.equality.postMessage({command:'LoadItem', diagram:diagram.name, item:item.name, leftLeg:leftSigs, rightLeg:rightSigs, equal});
	}
	...
	downloadDiagramData(name, cache, fn, timestamp)
	{
	...
	{
		const name = cell instanceof Cell ? cell.name : cell;
		if (handler)
		{
			if (!cellToCallbacks.has(name))
				cellToCallbacks.set(name, []);
			const handlers = cellToCallbacks.get(name);
...
class Toolbar
{
	...
	showDiagramToolbar(diagram, btns)
	{
		...
		btns.push(D.getIcon('cell', 'cell', e => setActive(e, 'cell', ee => Cat.D.elementTool.Assert.html(ee)), {title:'Assertions'}));
...
class CellTool extends ElementTool
{
	constructor(type, headline)
	{
		super('Assert', D.toolbar.help, headline, ['search']);
		this.hasDiagramOnlyButton = false;
		Object.defineProperties(this,
		{
			descriptionElt:		{value: null,		writable: true},
...
//							D.toolbar.help.querySelector('#new-basename').focus();
	...
	getIcon(name, buttonName, clickme, options = {})	// options = {title, scale, id, repeatCount, help}
	{
		...
		return H3.span('.icon', H3.svg(		{viewbox:"0 0 320 320", width:`${inches}in`, height:`${inches}in`},
											...
											H3.rect('.icon-background', {x:"0", y:"0", width:D.default.icon, height:D.default.icon}),
											H3.use({href:`#icon-${buttonName}`}),
											H3.rect('.btn', {x:"0", y:"0", width:D.default.icon, height:D.default.icon, onclick})), args);
	}
	...
	getObjects(ary)
	{
		...
		{
			const elt = ary[i];
			if ((elt instanceof IndexObject || elt instanceof DiagramText) && !(elt.name in elts))
				elts.add(elt);
...
class LogSection extends Section
{
	constructor(parent)
	{
		...
		this.diagram = null;
		this.logElt = null;
		const div = H3.div(H3.table(H3.tr(H3.td('.buttonBarRight', D.getIcon('logClear', 'delete', _ => Cat.R.diagram.clearLog(), {title:'Clear log'}),
	...
	constructor()
	{
		...
		const elements = [
			...
			H3.div(H3.table(H3.tr(H3.td(	D.getIcon('ttyClear', 'delete', _ => D.removeChildren(this.out), {title:'Clear output'}),
											D.getIcon('log', 'download-log', e => Cat.D.downloadString(this.out.innerHTML, 'text', 'console.log'), {title:'Download tty log file'}), '.buttonBarRight'))),
			...
			H3.div(H3.table(H3.tr(H3.td(	D.getIcon('ttyErrorClear', 'delete', _ => D.removeChildren(this.error)),
											D.getIcon('error', 'download-error', e => Cat.D.downloadString(this.error.innerHTML, 'text', 'console.err'), {title:'Download error log file'}), '.buttonBarRight'))),
...
class Element
{
	...
	help(root)
	{
		...
		H3.tr(H3.td('User:', '.left'), H3.td(this.diagram ? this.diagram.user : '', '.left'))].map(row => root.appendChild(row));
	}
...
class CatObject extends Element
{
	...
	{
		super.help(root);
		root.appendChild(H3.tr(H3.td('Category:'), H3.td(this.category.properName)));
	}
	decrRefcnt()
	{
		super.decrRefcnt();
...
class FiniteObject extends CatObject	// finite, explicit size or not
{
	...
	{
		super.help(root);
		root.appendChild(H3.tr(H3.td('Finite object'), H3.td('size' in this ? 'size: ' + this.size.toString() : 'indeterminate size')));
	}
	json()
	{
		const d = super.json();
...
class MultiObject extends CatObject
{
	...
	help(root, hdr)
	{
		...
		this.objects.map(o => root.appendChild(H3.tr(H3.td(o.getHtmlRep()))));
	}
	decrRefcnt()
	{
		super.decrRefcnt();
...
class ProductObject extends MultiObject
{
	...
	help(root)
	{
		super.help(root, H3.tr(H3.td('Type:'), H3.td(this.dual ? 'Coproduct' : 'Product')));
	}
	getFactor(factor)
	{
		if (factor === -1)
...
class PullbackObject extends ProductObject
{
	...
	help(root)
	{
		super.help(root, H3.tr(H3.td('Type:', H3.td('Pullback object'))));
	}
	decrRefcnt()
	{
		super.decrRefcnt();
...
class HomObject extends MultiObject
{
	...
	help(root)
	{
		super.help(root, H3.tr(H3.td('Type'), H3.td('Hom')));
	}
	homDomain()
	{
...
class DiagramText extends Element
{
	...
	help(root)
	{
		...
		root.appendChild(H3.tr(H3.td(div, {colspan:2})));
	}
...
class TensorObject extends MultiObject
{
	...
	help(root)
	{
		super.help(root, H3.tr(H3.td('Type:'), H3.td('Tensor')));
	}
	getGraph(data = {position:0}, first = true)
	{
...
class IndexObject extends CatObject
{
	constructor(diagram, args)
	{
		...
		isGUI && this.constructor.name === 'IndexObject' && D.emitElementEvent(diagram, 'new', this);
	}
	help(root)
	{
		return this.to.help(root);
	}
	json()
	{
		let a = super.json();
...
class Assertion extends Element
{
	...
	setSignature()
	{
		this.signature = this.cell.signature;
	}
	decrRefcnt()
	{
		super.decrRefcnt();
	...
	{
		const legs = [[], []];
		if (!ary.reduce((r, m) => r && m instanceof IndexMorphism, true))
	...
	static HasForm(diagram, left, right)
	{
		...
		const cell = diagram.getCell(Cell.Name(diagram, left, right));
...
class Action extends CatObject
{
	...
	help(root)
	{
		super.help(root, H3.tr(H3.td('Type:'), H3.td('Action')));
	}
...
class DetachDomainAction extends Action
{
	...
	{
		if (diagram.isEditable() && ary.length === 1 && ary[0] instanceof IndexMorphism && !(ary[0] instanceof DiagramComposite) && ary[0].refcnt === 1)
		{
			const from = ary[0];
			...
			if (soFar)
				return ![...from.domain.nodes].reduce((r, cell) => r || ([...cell.left, ...cell.right].includes(from) && diagram.getAssertion(cell.name)), false);
			return soFar;
		}
		return false;
	}
...
class DeleteAction extends Action
{
	...
	action(e, diagram, ary)
	{
		...
		const elements = ary.map(elt => elt instanceof Cell ? diagram.getAssertion(elt.name) : elt);		// convert cells to assertions
		diagram.deselectAll();
...
class HelpAction extends Action
{
	...
	html(e, diagram, ary)
	{
		...
		const table = H3.table('##help-body');
...
class AlignHorizontalAction extends Action
{
	...
	getItems(ary)
	{
		return ary.filter(s => s instanceof IndexObject || s instanceof DiagramText);
	}
	hasForm(diagram, ary)	// one object
	{
		if (!diagram.isEditable())
...
class ActionAction extends Action
{
	...
	html(e, diagram, ary)
	{
		...
		const ops = new Set();
		const namedBare = new Set();
		this.getBareElements(named, namedBare, ops);
		const unused = selected.filter(elt => !namedBare.has(elt));
		...
		if (unused.length > 0)
		{
			...
			elements.push(	H3.h5('Unused Elements'),
		...
		if (namedBare.size > 0)
		{
			...
			elements.push(	H3.h5('Generalized Elements'),
		...
		selected.map(elt => this.getBareElements(elt, selectedBare, ops));
		const notGeneralized = [];
		namedBare.forEach(elt => !selectedBare.has(elt) && notGeneralized.push(elte));
		if (notGeneralized.length > 0)
		{
			const html = notGeneralized.map(elt => elt.getHtmlRep());
			html.map(mh => mh.classList.add('element'));
			elements.push(	H3.h5('Not Generalized'),
							H3.span('.smallPrint', 'These bare elements are not generalized for the action.  Their diagrams will be referenced by the new action.'), ...html);
		}
	...
	_build(diagram, elt, nameMap)
	{
		...
		{
			this._build(diagram, elt.domain, nameMap);
			this._build(diagram, elt.codomain, nameMap);
		}
		if (elt.constructor.name === 'CatObject')
		{
			const basename = nameMap.get(elt.name);
			newElt = diagram.get(elt.constructor.name, {basename});
		}
		else if (elt.constructor.name === 'Morphism')
		{
			const basename = nameMap.get(elt.name);
			const domain = diagram.getElement(nameMap.get(elt.domain.name));
			const codomain = diagram.getElement(nameMap.get(elt.codomain.name));
			newElt = diagram.get(elt.constructor.name, {basename, domain, codomain});
		}
		else if (elt instanceof NamedObject || elt instanceof NamedMorphism)
		{
			const source = this._build(diagram, elt.source, nameMap);
			const basename = nameMap.has(elt.name) ? nameMap.get(elt.name) : elt.basename;
			newElt = diagram.get(elt.constructor.name, {basename, source});
		}
		else if (elt.constructor.name === 'Identity' || elt.constructor.name === 'Evaluation')
		{
			const domain = this._build(diagram, elt.domain, nameMap);
			newElt = diagram.get(elt.constructor.name, {domain});
		}
		else if (elt instanceof MultiObject)
		{
			const objects = elt.objects.map(o => this._build(diagram, o, nameMap));
			newElt = diagram.get(elt.constructor.name, {objects, dual:elt.dual});
		}
		else if (elt instanceof MultiMorphism)
		{
			const morphisms = elt.morphisms.map(m => this._build(diagram, m, nameMap));
			newElt = diagram.get(elt.constructor.name, {morphisms, dual:elt.dual});
		}
		else if (elt instanceof FactorMorphism)
		{
			const args = {factors:elt.factors, dual:elt.dual};
			const side = elt.dual ? 'codomain' : 'domain';
			args[side] = diagram.getElement(nameMap.get(elt[side].name));
			newElt = diagram.get(elt.constructor.name, args);
		}
		else if (elt instanceof LambdaMorphism)
		{
			const preCurry = this._build(diagram, elt.preCurry, nameMap);
			newElt = diagram.get(elt.constructor.name, {preCurry, domFactors:elt.domFactors, homFactors:elt.homFactors});
		}
		else if (elt instanceof Distribute || elt instanceof Dedistribute)
		{
			const domain = this._build(diagram, elt.domain, nameMap);
			newElt = diagram.get(elt.constructor.name, {domain, side:elt.side});
		}
		nameMap.set(elt.name, newElt.basename);
		return newElt;
	}
	action(e, diagram, ary, namedBare, ops, references)
	{
		...
		try
		{
			...
			{
				nameMap.set(elt.name, names[i]);
				this._build(action, elt, nameMap);
			});
			const named = this._build(action, namedSource, nameMap);
			action.named = named;
			let xy = new D2();
			action.makeSVG();
			...
			xy = xy.add(yDelta);
			let history = `Source diagram ${diagram.name} on ${Date()}\nElements: `;
			...
			const txt = action.placeText(history, xy.sub(xDelta), 12, 'normal', false);
			xy = xy.add(yDelta2);
			...
			{
				action.placeMorphism(named.source, xy, null, false);
...
class Category extends CatObject
{
	...
	{
		super.help(root);
		root.appendChild(H3.tr(H3.td('Category'), H3.td(`${this.elements.size} elements and ${this.actions.size} actions.`)));
	}
	info()
	{
		const a = super.info();
	...
	help(root, from = null)
	{
		...
		if (this.isEditable() && from && from.to.refcnt === 1)
		{
			...
			if (from.domain.refcnt === 2)
			{
				...
				root.appendChild(H3.tr(H3.td('Domain:'), H3.td(domainElt)));
			}
			if (from.codomain.refcnt === 2)
			{
				...
				root.appendChild(H3.tr(H3.td('Codomain:'), H3.td(codomainElt)));
			}
		}
		if (!domainElt)
		...
		if (R.canFormat(this))
		{
			...
			root.appendChild(H3.tr(H3.td(H3.small('Domain')), H3.td(H3.small('Codomain'))));
			root.appendChild(H3.tr(	H3.td(domain.properName + (domain.getBase() !== domain ? ` [${domain.getBase().properName}]` : ''), '.smallBold'),
...
//					root.appendChild(H3.tr(H3.td(typeof i === 'number' ? i.toString() : i), H3.td(typeof d === 'number' ? d.toString() : d), H3.td(editDataBtn)));
					// TODO delete
					root.appendChild(H3.tr(H3.td(typeof i === 'number' ? i.toString() : i), H3.td(typeof d === 'number' ? d.toString() : d)));
				}
			};
			root.appendChild(H3.tr(H3.th('Data in Morphism', {colspan:2})));
...
class Identity extends Morphism
{
	...
	{
		super.help(root);
		root.appendChild(H3.tr(H3.td('Type:'), H3.td('Identity')));
	}
	getGraph(data = {position:0})
	{
...
class NamedObject extends CatObject	// name of an object
{
	...
	{
		super.help(root);
		root.appendChild(H3.tr(H3.td('Named object of:'), H3.td(this.source.properName)));
	}
	getBase()
	{
		if ('base' in this)
...
class NamedMorphism extends Morphism	// name of a morphism
{
	...
	{
		super.help(root);
		root.appendChild(H3.tr(H3.td('Named morphism of:'), H3.td(this.source.properName)));
	}
	loadItem()	// don't call in Morphism constructor since signature may change
	{
		super.loadItem();
...
class IndexMorphism extends Morphism
{
	constructor(diagram, args)
	{
		...
		isGUI && this.constructor.name === 'IndexMorphism' && D.emitElementEvent(diagram, 'new', this);
	}
	setHomsetIndex(args)
	{
	...
	help(root)
	{
		this.to.help(root, this);
		if (this.isEditable())
		{
			const inputArgs = {type:"number", onchange:e => this.updateHomsetIndex(Number.parseInt(e.target.value)), min:0, max:100, width:2, value:this.homsetIndex};
			root.appendChild(H3.tr(H3.td('Homset Index'), H3.td(H3.input('.in100', inputArgs))));
		}
	}
	decrRefcnt()
	{
...
class Cell extends DiagramCore
{
	...
	help(root)
	{
		...
		switch(this.commutes)
		{
			...
			case 'notequal':
				...
				break;
			case 'hidden':
				buttons.push(D.getIcon('delete', 'delete', e => this.action(e, 'show'), {title:'Reveal this cell'}));
				break;
			case 'composite':
			case 'named':
			case 'computed':
		...
		root.appendChild(H3.table(rows));
	}
	setCommutes(com)
	{
		this.commutes = com;
	...
	getSVG(node)
	{
		...
		this.svg = svg;
		this.setGlow();
		this.diagram.selected.filter(elt => elt instanceof Assertion && elt.name === this.name).map(assert => assert.showSelected());
	}
	removeSVG()
	{
	...
	static Get(diagram, left, right)
	{
		...
		else
		{
			...
			diagram.domain.cells.set(cell.name, cell);
		}
		return cell;
	}
...
class DiagramComposite extends IndexMorphism
{
	constructor(diagram, args)
	{
		super(diagram, args);
		...
		this.constructor.name === 'DiagramComposite' && this.setSignature();
...
class IndexCategory extends Category
{
	...
	{
		super.help(root);
		root.appendChild(H3.tr(H3.td('Type:'), H3.td('Index Category')));
	}
	clear()
	{
		super.clear();
	...
	findCells(diagram)
	{
		...
		this.cells.forEach(cell => cell.register());
	}
	loadCells(diagram)
	{
...
class MultiMorphism extends Morphism
{
	...
	help(root, hdr)
	{
		super.help(root);
		root.appendChild(hdr);
		this.morphisms.map(m => root.appendChild(H3.tr(H3.td({colspan:2}, m.getHtmlRep()))));
	}
	decrRefcnt()
	{
		super.decrRefcnt();
...
class Composite extends MultiMorphism
{
	...
	help(root)
	{
		super.help(root, H3.tr(H3.td('Type:'), H3.td('Composite')));
	}
	getDecoration()
	{
...
class ProductMorphism extends MultiMorphism
{
	...
	help(root)
	{
		return super.help(root, H3.tr(H3.td('Type:'), H3.td(this.dual ? 'Coproduct' : 'Product')));
	}
	getGraph(data = {position:0})
	{
		const graph = super.getGraph(data);
...
class ProductAssembly extends MultiMorphism
{
	...
	help(root)
	{
		return super.help(root, H3.tr(H3.td('Type:'), H3.td(`${this.dual ? 'Co' : 'P'}roduct assembly`)));
	}
	getGraph(data = {position:0})
	{
		const graph = super.getGraph(data);
...
class FactorMorphism extends Morphism
{
	...
	{
		super.help(root);
		root.appendChild(H3.tr(H3.td('Type:'), H3.td(`${this.dual ? 'Cofactor' : 'Factor'} morphism: ${this.factors}`)));
	}
	json()
	{
		const a = super.json();
...
class LambdaMorphism extends Morphism
{
	...
	{
		super.help(root);
		root.appendChild(H3.tr(H3.td('Type:'), H3.td('Lambda')));
		root.appendChild(H3.tr(H3.td('Pre-Curry:'), H3.td(this.preCurry.properName)));
		root.appendChild(H3.tr(H3.td('Domain Factors:'), H3.td(this.domFactors)));
		root.appendChild(H3.tr(H3.td('Codomain Factors:'), H3.td(this.homFactors)));
	}
	json()
	{
		const a = super.json();
...
class Evaluation extends Morphism
{
	...
	{
		super.help(root);
		root.appendChild(H3.tr(H3.td('Type:'), H3.td('Evaluation')));
	}
	getGraph()
	{
...
class Distribute extends Morphism
{
	...
	{
		super.help(root);
		root.appendChild(H3.tr(H3.td('Type:'), H3.td('Distribute')));
	}
	hasInverse()
	{
		return true;
...
class Dedistribute extends Morphism	// TODO what about side?
{
	...
	{
		super.help(root);
		root.appendChild(H3.tr(H3.td('Type:'), H3.td('Dedistribute')));
	}
	hasInverse()
	{
		return true;
...
class Functor extends Morphism
{
	...
	{
		super.help(root);
		root.appendChild(H3.tr(H3.td('Type:'), H3.td('Functor')));
	}
}
...
class Diagram extends Functor
{
	...
	addSelected(elt)
	{
		if (!this.selected.includes(elt))	// not already selected
		{
			...
			if (elt instanceof IndexObject || elt instanceof DiagramText)
				elt.finishMove();
			else if (elt instanceof IndexMorphism)
			{
	...
	updateDragObjects(majorGrid)
	{
		...
		this.selected.map(elt =>
		{
			...
			else if (elt instanceof IndexObject || elt instanceof DiagramText)
				dragObjects.add(elt);
	...
	get(prototype, args)
	{
		try
		{
			...
			if (!(elt instanceof IndexMorphism) && !(elt instanceof Assertion) && 'loadItem' in elt)
				elt.loadItem();
			return elt;
		}
		catch(x)
	...
	moveElements(offset, ...elements)
	{
		...
		elements.map(elt =>
		{
			if (elt instanceof DiagramText || elt instanceof IndexObject)
				items.add(elt);
			else if (elt instanceof IndexMorphism)
			{
...
class ActionDiagram extends Diagram
{
	constructor(diagram, args)
	{
		...
		if (namedMorph)
		{
			...
			if (named === undefined)
				throw `cannot find named element ${nuArgs.named}`;
			if (!(named instanceof NamedMorphism || named instanceof NamedObject))
				throw `provided element is not a named element: ${named.name}`;
		}
		const attributes = new Map('attributes' in nuArgs ? nuArgs.attributes : []);
	...
	help(root)
	{
		return super.help(root, H3.tr(H3.td('Type:'), H3.td('Action')));
	}
	action(e, diagram, ary)
	{
